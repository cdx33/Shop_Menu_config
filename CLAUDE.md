...
Turns a menu design (website URL, screenshot(s), Figma) + a `storekey` into paste-ready
`MenuItem[]` JSON for the Venn Store Editor document **"Shop Menus - Dashboard only"**.
...
- LLMs are used ONLY inside extraction adapters (image/Figma → `ExtractedMenuNode` tree).
  Resolution, matching, validation and emission are plain code. Never let a model write an ID.
...
- `vercel dev` — review app locally (Vite frontend + `api/` functions)
